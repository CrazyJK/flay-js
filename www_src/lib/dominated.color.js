export const GREY_RGB_ARRAY = Array.from({ length: 256 }).map((v, i) => [
    i,
    i,
    i,
]);
...
export default async function getDominatedColors(src, opts) {
    ...
    function getRGBAs(data, ignoreRGBs, offset) {
        ...
        for (
            ...
            if (offset > 1) {
                rgbComponent = rgbComponent.map((x) =>
                    Math.round(Math.floor(x / offset) * offset)
                );
            }

            // skip colors in the ignore list
            if (
                ignoreRgbStrings.length > 0 &&
                ignoreRgbStrings.includes(rgbComponent.join(','))
            ) {
                continue;
            }
