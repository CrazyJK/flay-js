...
function initCondition() {
    ...
    const subti = LocalStorageItem.getBoolean(
        'flay.home.cond.subtitles',
        false
    );
    const favor = LocalStorageItem.getBoolean('flay.home.cond.favorite', true);
    const nofav = LocalStorageItem.getBoolean(
        'flay.home.cond.nofavorite',
        true
    );
...
function showFlay() {
    ...
    $('.container-flay .flay .flay-modified').html(
        DateUtils.format(FlayFiles.lastModified(currentFlay), 'yy.mm.dd')
    );
    $('.container-flay .flay .flay-access  ').html(
        currentFlay.video.lastAccess > 0
            ? DateUtils.format(currentFlay.video.lastAccess, 'yy.mm.dd')
            : '--.--.--'
    );
    ...
    $('.container-flay .flay .flay-comment ')
        .toggleClass(
            'comment-empty',
            StringUtils.isBlank(currentFlay.video.comment)
        )
        .html(
            StringUtils.isBlank(currentFlay.video.comment)
                ? 'Comment'
                : currentFlay.video.comment
        )
        .show();
    $('.container-flay .flay .flay-comment-input').hide();
    $(
        '.container-flay .flay .flay-info-rank input#flay-rank' +
            currentFlay.video.rank
    ).prop('checked', true);
    $('.container-flay .flay .flay-movie').html(
        (function () {
            let html =
                currentFlay.files.movie.length > 1
                    ? currentFlay.files.movie.length + 'V '
                    : '';
            let sumSize = 0;
    ...
    $('.container-flay .flay .flay-subtitles')
        ...
        .toggleClass(
            'subtitles-empty',
            currentFlay.files.subtitles.length === 0
        );
    ...
    for (const actressName of currentFlay.actress) {
        ...
        $(`<div class="flay-actress">
			<label class="flay-actress-favorite">
				<input type="checkbox" name="actressFavorite" class="sr-only" ${
			...
			<label class="flay-actress-birth"    >${(actress.birth || '').replace(
                /年|月|日/g,
                (match, offset, string) => '<small>' + match + '</small>'
            )}</label>
			<label class="flay-actress-body"     >${(actress.body || '').replace(
                / - /g,
                (match) => '<small>' + match.trim() + '</small>'
            )}</label>
			<label class="flay-actress-height"   >${
                actress.height || '' ? actress.height + '<small>cm</small>' : ''
            }</label>
			<label class="flay-actress-debut"    >${
                actress.debut || '' ? actress.debut + '<small>d</small>' : ''
            }</label>
		</div>`)
...
function addEventListener() {
    ...
    $(
        '.flay-navbar input[type="checkbox"], .flay-navbar input[type="radio"]'
    ).on('change', filterFlay);

    // ==== flay event
    // comment change
    $('.container-flay .flay .flay-comment').on('click', () => {
        $('.container-flay .flay .flay-comment').hide();
        $('.container-flay .flay .flay-comment-input')
            .val($('.container-flay .flay .flay-comment').text())
            .show()
            .focus();
    });
    // comment save
    $('.container-flay .flay .flay-comment-input').on('keyup', (e) => {
        e.stopPropagation();
        if (e.keyCode === 13) {
            let commentText = $('.container-flay .flay .flay-comment-input')
                .val()
                .trim();
    ...
    // actress favorite
    $('.flay-info-actress').on(
        'change',
        'input[name="actressFavorite"]',
        (e) => {
            const actress = $(e.target)
                .closest('.flay-actress')
                .data('actress');
    ...
    $('.flay-opus').on('click', (e) => {
        window.open(
            '/api/flay/' + currentFlay.opus,
            'video-' + currentFlay.opus,
    ...
    $(document).on('wheel keyup', (e) => {
        ...
        console.debug('pagination event', eventCode);
        switch (eventCode) {
            ...
            case 34: // page Down -> next 10
                goPage(
                    Math.min(currentOpusIndex + 10, filteredOpus.length - 1)
                );
                break;
            case 36: // Home -> 1
                goPage(0);
